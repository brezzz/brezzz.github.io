            ...
            <form class='quizQuestions' >
                <fieldset>
                    <label class='answerContainer'>${Quiz[currentQuestion].multiple_choice[0]}
                        <input type="radio" name="option" value='${Quiz[currentQuestion].multiple_choice[0]}'checked='checked' required></input><span class="checkmark"></span>
...
// resources menu. subject to change in desktop version. 'mobile-resources-menu'
$(document).ready(function () {
  $(document).on('click', '.menu-close', function () {
    $('.resources-menu').fadeOut();
  });
  $(document).on('click', '.menu-open', function () {
    $('.resources-menu').fadeIn();
  });
});
...
function inputHint() {
  document.getElementById('Hint').title = `${Quiz[currentQuestion].answer}`
}
...
function determindedIncorrect() {
  $('#quizPage').html(
    `<div class="feedback">
            ...
            <h2>That was the <em style='color:red'>incorrect</em> choice!. The correct answer was <em id='correctAnswer'>${Quiz[currentQuestion].answer}</em></h2> 
            <p>
                Here's a resource to learn more about
 <a href = '${Quiz[currentQuestion].topic_url} '><p class="topic"> ${Quiz[currentQuestion].topic}</p> </a>
            </p>
